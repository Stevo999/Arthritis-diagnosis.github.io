...
def checkdisease(request):
  ...
  diseaselist1=['Osteoarthristis','Arthritis']
     ...
     return render(request,'patient/checkdisease/checkdisease.html', {"list2":alphabaticsymptomslist})
      ...
      if (inputno == 0 ) :
        ...
        if predicted_disease in Rheumatologist :
           ...
           consultdoctor = " "
           print("No chances of osteoarthritis found")
        ...
        #changes
        if(predicted_disease in diseaselist1):
          predicted_disease=predicted_disease
        elif (predicted_disease not in diseaselist1):
         predicted_disease=''
         confidencescore=0
...
def consult_a_doctor(request):
    if request.method == 'GET':
        doctortype = request.session.get('doctortype')  # Get the doctortype from session
        if doctortype == 'Rheumatologist':
            dobj = doctor.objects.filter(specialization='Rheumatology')
            return render(request, 'patient/consult_a_doctor/consult_a_doctor.html', {"dobj": dobj})
    return HttpResponseBadRequest()  # Handle invalid requests
